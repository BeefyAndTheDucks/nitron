...
pub fn run(event_loop: EventLoop<()>, vertices: Vec<Vert>, indices: Vec<u32>) {
    let context_lock = VULKAN_CONTEXT.get().expect("Vulkan context not initialized");
    let mut context = context_lock.write().expect("RwLock poisoned");
    ...
    let vs = vertex_shader::load(context.device.clone()).expect("failed to create shader module");
    let fs = fragment_shader::load(context.device.clone()).expect("failed to create shader module");
